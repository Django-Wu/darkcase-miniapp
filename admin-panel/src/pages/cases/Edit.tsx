...
export const CaseEdit: React.FC = () => {
  ...
  useEffect(() => {
    if (caseData?.data) {
      ...
      reset({
        ...
        crimeType: caseItem.crime_type || caseItem.crimeType || [],
        tags: caseItem.tags || [],
        status: caseItem.status,
        victims: caseItem.victims,
        rating: caseItem.rating,
        poster: caseItem.poster,
        backdrop: caseItem.backdrop,
        videoUrl: caseItem.video_url || caseItem.videoUrl,
        timeline: caseItem.timeline || [],
        facts: caseItem.facts || [],
      })
      setTimelineItems(caseItem.timeline || [])
      setFacts(caseItem.facts && caseItem.facts.length > 0 ? caseItem.facts : [''])
    }
  }, [caseData, reset])

  const updateMutation = useMutation({
    mutationFn: (data: CaseFormData) => casesAPI.update(id!, data),
    onSuccess: () => {
      navigate('/cases')
    },
  })
  ...
  if (isLoading) {
    return <div className="text-center text-white/60 py-12">Загрузка...</div>
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Редактировать кейс</h1>
          <p className="text-white/60">Измените информацию о кейсе</p>
        </div>
      </div>
...
// Reuse FileUploadField from Create
...
}> = ({ label, type, value, uploading, onUpload }) => {
